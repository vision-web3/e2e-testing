...
test_timeout = int(os.getenv('PYTEST_TIMEOUT', 1000))  # 5 minutes
...
def setup_module(request, stack_id, worker_id="gw0"):
    ...
    #os.environ['VISION_ENV_FILE'] = '/Users/joe.matthew/Workspace/cd-workflows/tests/testnet/e2e-test-config.env'
    ...
    if os.getenv('VISION_ENV_FILE', None) is None:
        port_offset = int(worker_id.replace(
            "gw", "")) if worker_id != "master" else 0
        #os.environ['PORT_OFFSET'] = str(port_offset)
        configure_nodes(
            {
                'ethereum_contracts': {
                    'args': '',
                    'port_group': port_offset
                },
                'service_node': {
                    'instance_count': 1,
                    'port_group': port_offset
...
def test_retrieve_token_balance(receiving_address):
    try:
        token_balance = pc.retrieve_token_balance(
            pc.Blockchain.ETHEREUM, pc.BlockchainAddress(receiving_address),
            pc.TokenSymbol('vsn'))
        assert token_balance is not None
        print(f'Token balance: {token_balance}')
    except pc.VisionClientError:
        pytest.fail("VisionClientError raised")


def test_retrieve_service_node_bids():
    try:
        service_node_bids = pc.retrieve_service_node_bids(
            pc.Blockchain.ETHEREUM, pc.Blockchain.BNB_CHAIN, False)
...
def test_token_transfer(receiving_address, private_key):
    try:
        token_transfer_response = pc.transfer_tokens(
            ...
            pc.BlockchainAddress(receiving_address), pc.TokenSymbol('vsn'),
    ...
    while not done:
        try:
            token_transfer_status = pc.get_token_transfer_status(
                ...
                token_transfer_response.service_node_address,
            ...
            if (token_transfer_status.source_transfer_status
                    is ServiceNodeTransferStatus.CONFIRMED
                    and token_transfer_status.destination_transfer_status
                    is DestinationTransferStatus.CONFIRMED):
                done = True
            else:
...
# @pytest.mark.parametrize('keystore', ['eth'], indirect=True)
# def test_deploy_vision_compatible_token(private_key):
#     try:
